...
module.exports = function fileExplorer (options) {
  ...
  for (selector in options.classes) {
    if (!css[selector]) throw new Error(`
      given:
        ".${selector}"
      supported css selectors:
        ".${Object.keys(css).join('"\n      ".')}"
    `)
    css[selector] = options.classes[selector]
  }
  ...
  function renderNode (data) {
    ...
    return renderedChildren.length ? yo`
      <div class=${css.container} onclick=${focus}>
        <span class=${css.node} onclick=${toggleExpand}>${caret}${data}</span>
        ${children}
      </div>
    ` : yo`<div class=${css.container} onclick=${focus}><span class=${css.node}>${data}</span></div>`
